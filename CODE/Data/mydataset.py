...
def make_dataset(imgroot ,segroot):
    ...
    for i in os.listdir(imgroot):
        img = os.path.join(imgroot, i.split('.')[0] + '.jpg')
...
class myDataset(data.Dataset):
    ...
    def adjustContrast(self, image, mask):
        factor = transforms.RandomRotation.get_params([0, 10])  
    ...
    def adjustBrightness(self, image, mask):
        factor = transforms.RandomRotation.get_params([1, 2]) 
